...
void Reserva::criaAnimal(const string &tipo, const int &x, const int &y)
{
    if (tipo != "C" && tipo != "O" && tipo != "L" && tipo != "G" && tipo != "M") {
        ...
        if (tipo == "C") {
            animais.push_back(new Coelho("C", constantesReader("SCoelho"), constantesReader("VCoelho"), 0, rand() % 4 + 1, x, y));
            animais[animais.size() - 1]->setId(totalCoisas++);
        } else if (tipo == "O") {
            animais.push_back(new Ovelha("O", constantesReader("SOvelha"), constantesReader("VOvelha"), 0, rand() % 5 + 4, x, y));
            animais[animais.size() - 1]->setId(totalCoisas++);
        } else if (tipo == "L") {
            animais.push_back(new Lobo("L", constantesReader("SLobo"), constantesReader("VLobo"), 0, 15, x, y));
            animais[animais.size() - 1]->setId(totalCoisas++);
        } else if (tipo == "G") {
            animais.push_back(new Canguru("C", constantesReader("SCanguru"), constantesReader("VCanguru"), 0, 10, x, y));
            animais[animais.size() - 1]->setId(totalCoisas++);
        } else if (tipo == "M") {
            animais.push_back(new AnimalMisterioso("M", constantesReader("SAnimalM"), constantesReader("VAnimalM"), 0, rand() % 9 + 2, x, y));
            animais[animais.size() - 1]->setId(totalCoisas++);
        }
    }
}
...
void Reserva::criaAnimalRandom(const string &tipo)
{
    ...
    if (tipo != "C" && tipo != "O" && tipo != "L" && tipo != "G" && tipo != "M") {
        ...
        if (tipo == "C") {
            animais.push_back(new Coelho("C", constantesReader("SCoelho"), constantesReader("VCoelho"), 0, rand() % 4 + 1, lRandom, cRandom));
            alimentos[alimentos.size() - 1]->setId(totalCoisas++);
        } else if (tipo == "O") {
            animais.push_back(new Ovelha("O", constantesReader("SOvelha"), constantesReader("VOvelha"), 0, rand() % 5 + 4, lRandom, cRandom)); //rand() % 5 + 4; numero entre 4 e 8 --
            alimentos[alimentos.size() - 1]->setId(totalCoisas++);
        } else if (tipo == "L") {
            animais.push_back(new Lobo("L", constantesReader("SLobo"), constantesReader("VLobo"), 0, 15, lRandom, cRandom));
            alimentos[alimentos.size() - 1]->setId(totalCoisas++);
        } else if (tipo == "G") {
            animais.push_back(new Canguru("G", constantesReader("SCanguru"), constantesReader("VCanguru"), 0, 10, lRandom, cRandom));
            alimentos[alimentos.size() - 1]->setId(totalCoisas++);
        } else if (tipo == "M") {
            animais.push_back(new AnimalMisterioso("M", constantesReader("SAnimalM"), constantesReader("VAnimalM"), 0, rand() % 9 + 2, lRandom, cRandom));
            alimentos[alimentos.size() - 1]->setId(totalCoisas++);
        }
    }
}
    ...
    for (int i = 0; i < animais.size(); i++) {
        ...
        if (animais[i]->getTipoAnimal() == "C") { // ANIMAL C COELHO
            ...
            if (animais[i]->getvAnimal() != 0) { //Se a vida for diferente de 0, ele faz toda a logica do animal
                ...
                if (numInstantes % 8 == 0) { // se passaram 8 instantes
                    ...
                    if (engravida == 1) { //engravidou com sucesso
                        ...
                        pair<int,
                             int> cf = createFilho(animais[i]->getX(), animais[i]->getY(), rand() % 8, rand() % 10 + 1); //apanha as coordenadas para o filho
                        animais.push_back(new Coelho(animais[i]->getTipoAnimal(), constantesReader("SCoelho"), constantesReader("VCoelho"), 0, rand() % 4 + 1, cf.first, cf.second));
                        animais[animais.size() - 1]->setId(totalCoisas++);
                    }
            ...
            if (animais[i]->getvAnimal() != 0) {
                ...
                if (numInstantes % 15 == 0) { // após 15 instantes
                    ...
                    pair<int,
                         int> cf = createFilho(animais[i]->getX(), animais[i]->getY(), rand() % 8, rand() % 12 + 1);

                    animais.push_back(new Ovelha(animais[i]->getTipoAnimal(), animais[i]->getSaude(), constantesReader("VOvelha"), 0, rand() % 5 + 4, cf.first, cf.second));
                    animais[animais.size() - 1]->setId(totalCoisas++);
                ...
                if (an != nullptr && pesoPerigoso) {   // Return true - faz Movimentação com animal na redondeza
                    ...
                    pair<int,
                         int> ca = pegaCoordAl(animais[i]->getX(), animais[i]->getY(), 4, "erva"); // retira coordenadas do alimento com cheiro tal
                    pair<int,
                         int> va = pegaValoresAl(animais[i]->getX(), animais[i]->getY(), 4, "erva"); // retira valores (nutri e toxi) do alimento com cheiro tal
                    animais[i]->fazMovimentacaoComer(ca.first, ca.second, va.first, va.second);
            ...
            if (animais[i]->getSaude() != 0) {
                ...
                if (numInstantes == nascer && !teveFilho) {
                    ...
                    animais.push_back(new Lobo(animais[i]->getTipoAnimal(), constantesReader("SLobo"), constantesReader("VLobo"), 0, 15, cf.first, cf.second));
                    animais[animais.size() - 1]->setId(totalCoisas++);
                    teveFilho = true;
                }
                ...
                if (an != nullptr) { // Return true - faz Movimentação com animal na redondeza
                    ...
                    pair<int,int> coordHunt = pegaCoordCaca(animais[i]->getId(), animais[i]->getX(), animais[i]->getY(), 5); //apanha a coordenada do animal mais pesado
            ...
            if (animais[i]->getvAnimal() != 0) {
                ...
                if (numInstantes % 20 == 0) { // apos 20 instantes, set ao peso a 20
                    animais[i]->setPeso(20);
                }

                if (numInstantes % 30 == 0) { // apos 30 instantes cria uma novo canguru entre 1 ou 3 casas de distancia

                    pair<int, int> cc = createFilho(animais[i]->getX(), animais[i]->getY(), rand() % 8, rand() % 3 + 1);

                    animais.push_back(new Canguru(animais[i]->getTipoAnimal(), constantesReader("SCanguru"), constantesReader("VCanguru"), 0, 10, cc.first, cc.second));
                    animais[animais.size() - 1]->setId(totalCoisas++);
            ...
            if (animais[i]->getSaude() != 0) {
                ...
                if (numInstantes % 10 == 0) {

                    pair<int,int> cf = createFilho(animais[i]->getX(), animais[i]->getY(), rand() % 8, rand() % 3 + 1);

                    animais.push_back(new AnimalMisterioso(animais[i]->getTipoAnimal(), constantesReader("SAnimalM"), constantesReader("SAnimalM"), 0, 15, cf.first, cf.second));
                    animais[animais.size() - 1]->setId(totalCoisas++);
                }
                ...
                achouComida = checkComida(animais[i]->getX(), animais[i]->getY(), 5, "fruta");
                ...
                if (an != nullptr && pesoPerigoso) {   // Return true - faz Movimentação com animal na redondeza
                    ...
                    pair<int,int> ca = pegaCoordAl(animais[i]->getX(), animais[i]->getY(), 4, "fruta"); // retira coordenadas do alimento com cheiro tal
                    pair<int,int> va = pegaValoresAl(animais[i]->getX(), animais[i]->getY(), 4, "fruta"); // retira valores (nutri e toxi) do alimento com cheiro tal
    ...
    for (auto &alimento : alimentos){
        ...
        if(alimento->getCheiro().size() == 2){
            if (alimento->getCheiro()[0] == comida || alimento->getCheiro()[1] == comida) {
                debug << "cheiro a: " << alimento->getCheiro()[0];
                debug << "cheiro a: " << alimento->getCheiro()[1];
